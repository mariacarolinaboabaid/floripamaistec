...
function calcularIMC(){
    ...
    const verificaStorage = localStorage.getItem("dadosIMC");

            if (verificaStorage !== null) {
                const historicoExistente = JSON.parse(verificaStorage);
                historico = historicoExistente;
            }

            historico.push(imc);

            const objetoString = JSON.stringify(historico);
            localStorage.setItem("dadosIMC", objetoString);
}
